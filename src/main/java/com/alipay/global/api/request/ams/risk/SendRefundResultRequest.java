...
/** The request of Ant Group's send refund result API. 调用蚂蚁集团退款结果通知接口的请求参数。 */
@Data
...
public class SendRefundResultRequest extends AlipayRequest<SendRefundResultResponse> {
  ...
  /**
   * A unique ID assigned by the merchant who provides the service or product directly to the buyer
   * to identify the refund. 由直接向买家提供服务或商品的商户分配的识别退款的唯一ID 。
   */
  private String referenceRefundId;
  /** The total amount of the actual refund. 实际退款总额。 */
  private Amount actualRefundAmount;
  /** Refund history for this transaction. 本次交易的退款记录。 */
